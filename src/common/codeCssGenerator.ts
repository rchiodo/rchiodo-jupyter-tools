...
// are ignoring themes.
/* eslint-disable no-multi-str, quote-props */
...
export class CodeCssGenerator {
    ...
    private async applyThemeData<T>(
        ...
        try {
            ...
            const editor = vscode.workspace.getConfiguration('editor', undefined);
            ...
            if (theme) {
                logInfo('Searching for token colors ...');
                ...
                if (tokenColors && fontFamily && fontSize) {
                    logInfo('Using colors to generate CSS ...');
                    result = applier({
                        tokenColors,
                        baseColors,
                        fontFamily,
...
`;
    ...
    private readTokenColors = async (themeFile: string): Promise<JSONArray> => {
        try {
            const tokenContent = await readLocalFile(themeFile);
            const theme = parse(tokenContent);
            let tokenColors: JSONArray = [];

            if (typeof theme.tokenColors === 'string') {
                const style = await readLocalFile(theme.tokenColors);
            ...
            logError('Error reading custom theme', e);
            return [];
        }
    };
        ...
        try {
            ...
            if (themeRoot) {
                ...
                if (found !== null) {
                    ...
                    logInfo(`Reading colors from ${themeFile}`);
                    return await this.readTokenColors(themeFile);
                }
            } else {
                logWarning(`Color theme ${theme} not found. Using default colors.`);
            }
        } catch (err) {
    ...
    private findBaseColors = async (theme: string): Promise<JSONObject | null> => {
        try {
            ...
            // Use the first result if we have one
            if (themeRoot) {
                logInfo(`Loading base colors from ${themeRoot} ...`);
                ...
                if (found !== null) {
                    ...
                    // the tokens in it
                    const themeFile = path.join(path.dirname(themeRoot), found.path);
                    logInfo(`Reading base colors from ${themeFile}`);
...
function logWarning(arg0: string) {
